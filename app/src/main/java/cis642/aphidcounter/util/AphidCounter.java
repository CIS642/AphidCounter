...
public class AphidCounter {
    ...
    public int countAphid(){
        ...
        Imgproc.drawContours(processedImage,contours,-1, new Scalar(255,255,255),-1);
        int counter = 0;
        for(int i = 0 ; i < contours.size(); i ++){
            if(Imgproc.contourArea(contours.get(i)) > 3.0) {
                ...
                if (rectArea < 200 && rectArea > 5 && rect.width < 20 && rect.height < 20 && contourRatio > 0.7){
                    //puts boxes on counted aphids.
                    //Core.rectangle(processedImage, new Point(rect.x,rect.y), new Point(rect.x+rect.width,rect.y+rect.height),new Scalar(255,255,255));
                    counter++;
                }
            }
        }
